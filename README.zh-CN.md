...
`from <relation> | emit { <field> }` 形式的 Runa Flow source，或格式版本 `2`
的规范 Runa Query IR。它也支持不可变 Observation Evidence 的写入、元数据
查询和有界 payload 回读。SQL 文本、PostgreSQL 协议和 PostgreSQL 客户端均不
受支持；旧 SQL parser、executor 和协议端点已经移除。
...
- Runa Query IR format version `2`
