...
This is the implemented engine read path. Because the in-memory tables hold one committed version per row and the single writer publishes only after WAL durability, a statement read observes the latest committed state at its start; the private write-set merge is what makes it read-your-writes. Deterministic tests cover read-your-writes, uncommitted invisibility to other readers and transactions, statement-level freshness (a second statement observes another transaction's intervening commit), and the insert/update/delete merge ordering.
