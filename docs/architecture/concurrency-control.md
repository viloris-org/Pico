...
The current public Flow/IR slice is read-only and has no transaction-control operations. The transaction ownership area (`txn`), the single-writer commit coordinator (`commit`), group commit, observed-version write-write conflict detection, bounded admission, and the durable commit watermark are implemented as a development slice and deterministically tested at the engine level. The Read Committed read path is implemented there too: transaction-aware scans and point reads (`selectAllTx`, `selectByPkTx`) merge the transaction's private write set over committed state, so an explicit transaction reads its own staged writes while every other reader sees only published commits. Version retention (roadmap Phase 5) is also implemented as a development slice: row versions carry a creation commit sequence, superseded versions are retained with a deletion interval, the snapshot registry tracks `oldest_active_snapshot_seq`, reads interpret only versions committed at or before their starting watermark, and reclamation frees only versions invisible to every active snapshot. Reclamation is driven by the maintenance path: the checkpoint's maintenance loop frees every retained version whose deletion interval lies entirely before the active-snapshot horizon (reported as `checkpoint_stats.retained_reclaimed`), and `Engine.retentionStats` exposes the retained, unreclaimable, and cumulative-reclaimed counts plus the oldest active watermark. The wire protocol does not yet expose any of this. Internal table mutation methods write through the coordinator, but they are not public transaction support. Until the implementation reaches the phase described here, transaction syntax or isolation must not be advertised through the protocol.
...
The engine implements this merge for the current in-memory tables: `selectAllTx`/`selectByPkTx` substitute the effective (last staged) write-set row for each touched key, hide staged deletes, and append private inserts. The result is fully owned and never aliases the write set or a live table row. `flow.executeTx` runs relation `emit` over that merged row set, so a transaction can read its own staged writes through the same projection, `where`, and `limit` pipeline as a committed read.
...
This is the implemented engine read path. A statement takes a fresh snapshot watermark at its start and interprets only versions committed at or before it; because the single writer publishes only after WAL durability, a statement read observes the latest committed state at its start, and the private write-set merge is what makes it read-your-writes. With version retention, a snapshot read keeps returning exactly the versions committed at or before its watermark even as later commits land (reads do not wait for writes). Deterministic tests cover read-your-writes, uncommitted invisibility to other readers and transactions, statement-level freshness (a second statement observes another transaction's intervening commit), the insert/update/delete merge ordering, snapshot-stable reads under later commits, and snapshot-safe reclamation.
...
The engine drives reclamation on its maintenance path rather than leaving it to a future compactor: the checkpoint's loop runs `reclaimRetainedVersions` after its flush+compact pass (bounded by the active-snapshot horizon), and the same engine boundary is available to the scheduler-fed maintenance worker. Reclamation mutates the per-table retained lists that in-flight scans iterate, so it must be serialized with statements (the engine statement lock in the threaded runtime; the single-writer model in the current server). Observability — retained, unreclaimable, and cumulative-reclaimed version counts plus the oldest active watermark (`Engine.retentionStats`) — is a day-one part of the boundary, and the checkpoint reports how many versions it reclaimed.
