...
验证 PostgreSQL 线协议的最小闭环。本文所述的并发接入、提交队列和 I/O
调度在相应模块落地并有回归后才成为实现保证。

本设计细化 ADR-0002、ADR-0005 与 ADR-0006，不能改变其中的以下约束：

1. Pico 是单机单实例服务，连接通过 PostgreSQL 线协议进入。
...
```mermaid
flowchart LR
  client["PostgreSQL 客户端"] --> reactor["net/reactor\n接收、发送、协议帧、背压"]
...
`BackendKeyData` 是 PostgreSQL 线协议的取消凭据，不能继续使用固定值。启动成功后，
Pico 为连接生成 `(process_id, secret_key)`，并将这个二元组映射到内部连接标识。收到
独立 TCP 连接上的 `CancelRequest` 时，reactor 只做常数时间查找和取消标记，不创建
SQL 执行上下文、不写入原连接、也不等待语句结束；找不到、密钥不匹配、连接已关闭
或语句代次已结束时静默关闭取消连接。标识或密钥绝不能在旧登记项完全撤销前复用，
否则延迟到达的取消请求可能取消无关连接。
...
次序发送。`ReadyForQuery` 的事务状态只能由 `txn` 给出：空闲为 `I`，显式事务中为
`T`，失败事务为 `E`；`net` 不得根据最后一条报文自行推断。

错误有明确作用域：长度、格式、消息顺序或启动阶段违规是协议错误，发送可行的
错误帧后关闭连接；不支持的 SQL 子集、绑定或执行错误只终结当前语句，并按协议回到
可接收下一语句的状态；传输 EOF 直接走关闭清理，不尝试发送错误。扩展查询协议加入
时，`Parse`/`Bind`/`Execute` 错误进入“忽略消息直到 `Sync`”的显式状态，且只在
`Sync` 后发送 `ReadyForQuery`。这条迁移不能散落在 socket 回调里，也不能用 Phase 0
的“每个错误后立即 ReadyForQuery”替代。
...
6. 使用真实 PostgreSQL 驱动发出 `CancelRequest`：错误凭据和已关闭连接不会影响
   其他连接；队列等待中的语句可取消；WAL 耐久边界后的取消不能撤销已提交效果。
7. 断开有显式事务的连接后未提交写集不可见；断开等待网络写出的连接不阻塞其他
   连接，且连接登记和取消凭据均被回收。

## 演进顺序

1. 保持 Phase 0 的顺序连接实现，补齐帧限制、错误状态和真实客户端协议回归。
