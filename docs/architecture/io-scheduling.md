...
This document refines the I/O scheduling rules in [Runtime, Connections, and Concurrency Control](runtime-and-concurrency.md). It is the RunaDB v1 target design; only the first slice is implemented as a development slice. The listener serves each Connection on its own thread behind the engine's statement-execution lock (Phase 6), result production streams through the execution program (`src/flow/program.zig`) in bounded batches with a flush per batch, and `src/runtime/scheduler.zig` implements the scheduler core — operation states, fixed per-class capacity, the bounded completion queue, and the tick discipline below — validated deterministically against a fake platform backend (`Scheduler` tests in the module). The socket-I/O reactor, per-Connection byte backpressure, reserved WAL slots, and compaction scheduling remain target work; until they land, the current per-Connection-thread handling remains the only implementation fact for network I/O.
