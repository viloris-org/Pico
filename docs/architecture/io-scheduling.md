...
This document refines the I/O scheduling rules in [Runtime, Connections, and Concurrency Control](runtime-and-concurrency.md). It is the RunaDB v1 target design; only the first slices are implemented as development slices. The listener serves each Connection on its own thread behind the engine's statement-execution lock (Phase 6), result production streams through the execution program (`src/flow/program.zig`) in bounded batches with a flush per batch, and `src/runtime/scheduler.zig` implements the scheduler core — operation states, fixed per-class capacity, the bounded completion queue, and the tick discipline below — validated deterministically against a fake platform backend (`Scheduler` tests in the module). Reserved WAL slots are implemented at the commit-admission boundary: `src/commit/coordinator.zig` holds a configurable WAL slot/byte reservation for every admitted commit request, released when the request's `txn_batch` frame is appended in a group-commit round or when the request is withdrawn/rejected before durability; when the reservation is exhausted, write admission is rejected with the retryable overload outcome `WalReservationExhausted` instead of letting WAL work pile up behind other I/O classes. Compaction scheduling has its first slice: `src/runtime/maintenance.zig` supplies a bounded maintenance worker — a real worker-thread platform backend that owns the scheduler, admits jobs through a bounded inbound queue (`MaintenanceFull` is the overload signal), feeds the scheduler's `maintenance` class up to its slot/byte budget and backpressure watermarks, runs jobs (compaction, checkpoint preparation, cleanup) on a dedicated thread outside the scheduler and the engine statement lock, honors cancellation at the admission and pre-run checkpoints, and releases every queued job with the cancelled outcome on teardown (`Scheduler.cancelAllQueued`). The scheduler-fed load regression in `src/net/runtime_test.zig` drives real LSM compaction through that path under concurrent writers and readers and verifies bounded admission, commit order, snapshot consistency, and exact restart recovery. The memtable-flush boundary has its first slice on the same path: `Engine.memtable_flush_entries` is the row-count flush boundary, `Engine.flushCandidates` exposes the tables that crossed it, and the scheduler-fed flush regression drives real LSM flushes through the maintenance worker under concurrent writers, then compacts the L0 files they produced. The socket-I/O reactor and per-Connection byte backpressure remain target work; until they land, the current per-Connection-thread handling remains the only implementation fact for network I/O, and checkpoint remains a statement-path action (its maintenance loop flushes, compacts, and reclaims retained versions).
...
| Commit-request or `critical` WAL slots | Do not admit another write statement on the connection; preserve order for queued requests | The single writer and WAL release enough capacity. Implemented at the commit-admission boundary: `src/commit/coordinator.zig` reserves one WAL slot plus the encoded `txn_batch` frame bytes per admitted request and rejects with `WalReservationExhausted` when the reservation is exhausted |
...
The first step is implemented: `src/runtime/scheduler.zig` defines the operation state machine (`queued -> submitted -> completed -> callback_ready -> released`, plus `cancelled` for queued cancellable work), fixed per-class slot/byte capacity, the bounded completion queue, class-priority submission (critical first, then foreground round-robin at owner granularity, then maintenance with the remaining per-tick budget), high/low backpressure watermarks with hysteresis, and the metrics listed under [Observability, Failure, and Acceptance](#observability-failure-and-acceptance). Its regressions cover callback depth, count balancing, next-tick-only submission of callback-produced work, capacity assertions, backpressure recovery, critical-slot reservation, budget exhaustion, completion-queue saturation, and cancellation, all against a fake platform backend. The next step is implemented at the commit path: the commit coordinator reserves WAL slot/byte capacity at admission (`WalReservationExhausted` is a retryable overload outcome), guaranteeing that every admitted commit can complete its WAL append and sync, with the reservation released on append, cancellation, validation failure, or append failure and covered by deterministic engine-level regressions. The next step is the maintenance worker: `src/runtime/maintenance.zig` is a real worker-thread platform backend for the scheduler with a bounded, mutex-guarded inbound admission queue (rejections are `error.MaintenanceFull`), cancellation honored at the admission and pre-run checkpoints, job-failure isolation, and teardown that releases every queued job exactly once; its deterministic regressions cover completion accounting, saturation, cancellation, failure isolation, teardown, and invalid-owner rejection, and the scheduler-fed compaction regression in `src/net/runtime_test.zig` drives real LSM compaction through the maintenance class under load. The remaining steps (socket I/O and per-Connection backpressure) still follow the same sequence.
