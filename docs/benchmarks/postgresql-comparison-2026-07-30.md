...
- RunaDB: worktree at `c7af212` (+ local WAL worktree), Zig 0.16.0,
  `ReleaseFast`, data under `zig-cache/runa-bench` on the same NVMe volume
...
| Workload | RunaDB (peak) | PostgreSQL (peak) | Positioning note |
| --- | ---: | ---: | --- |
| Autocommit `INSERT`, 50,000 ops | **140,029** ops/s, WAL sync **off** | **1,302** TPS, durable on NVMe | Not comparable: RunaDB is in-process and non-durable. |
| Primary-key `SELECT`, 50,000 ops | **1,308,215** ops/s | **28,360** TPS (index scan) | Not comparable: RunaDB bypasses network/protocol. |
...
| Primary-key `SELECT`, 10,000 ops | **1,290,228** ops/s | **27,151** TPS | RunaDB remains in-process. |
...
RunaDB 50,000-operation transactional insert with WAL sync off (peak):
...
| Workload | RunaDB range | PostgreSQL range (NVMe) |
| --- | ---: | ---: |
...
RunaDB creates `bench_items (id INT PRIMARY KEY, name TEXT)`, runs an autocommit
...
initdb -D "$PGDATA" --auth-local=trust -U runadb
...
```sh
export PGHOST="$PGSOCK" PGPORT=55432 PGUSER=runadb PGDATABASE=postgres
...
- Durable autocommit insert peak on this host: RunaDB **~1.8k ops/s** vs
  PostgreSQL **~1.5k TPS** under the methods above — still **not** an
  apples-to-apples protocol comparison.

Do not promote RunaDB as a general PostgreSQL performance replacement. RunaDB
bypasses network and protocol in these benches; PostgreSQL includes them. RunaDB
