# RunaDB SQL Administrative Extensions: RUNADB STATUS / CONFIG / SHUTDOWN
...
```runadbsql
RUNADB CONFIG;
RUNADB CONFIG durability_level;
RUNADB CONFIG durability_level = 'sync';
```
...
2. `RUNADB STATUS`, `RUNADB CONFIG`, and `RUNADB SHUTDOWN` are self-describing.
3. The syntax makes RunaDB’s distinction from PostgreSQL explicit.
4. Existing query messages are reused; no new handshake messages are needed.
5. The `RUNADB-` namespace leaves room for future administrative statements.

## Consequences

- Add `RUNADB` tokenization and parsing in `src/sql/parse.zig`.
- Add `Stmt.runadb_stmt` (or an equivalent structure) in `src/sql/ast.zig`.
- Route execution through `src/sql/exec/runadb.zig` from `src/sql/exec.zig`.
- Administrative statements do not write user data or pass through WAL. `RUNADB STATUS` uses the current snapshot; `RUNADB CONFIG` initially changes runtime state only.
- `RUNADB SHUTDOWN` performs the actual shutdown only after sending `command_complete`.
...
1. Define `RunaDBStmt` with `status`, `config`, and `shutdown` variants.
2. Parse the `RUNADB` keyword and statements.
3. Implement `src/sql/exec/runadb.zig`.
4. Route `Stmt.runadb_stmt` to the executor.
