# RunaDB-Native Protocol and RunaDB SQL; No PostgreSQL Compatibility Promise

RunaDB is an independent, single-node, lightweight, network-accessible OLTP database. Its external interfaces are the **RunaDB wire protocol** and **RunaDB SQL**; RunaDB defines their semantics, versions, and client ecosystem. RunaDB makes no promise of PostgreSQL wire-protocol, dialect, type-system, error-code, driver, or tool compatibility.
...
1. RunaDB must define its product boundary rather than being pulled toward PostgreSQL protocol and behavioral compatibility.
...
- **RunaDB-native ecosystem (adopted)**: The sole product contract is the versioned RunaDB protocol, RunaDB SQL, official client tools, and a public support matrix.

## Consequences

- New or changed client capabilities must define a RunaDB protocol version, RunaDB SQL support matrix, error model, and official-client compatibility scope.
- Network, SQL, and client-tool regression tests use the official RunaDB CLI/driver; `psql` and PostgreSQL drivers are no longer product acceptance criteria.
- The PostgreSQL Frontend/Backend Protocol may remain only as an explicitly unstable migration adapter and must be removed after the RunaDB-native protocol is available.
- `src/net` targets the RunaDB wire protocol and `src/sql` targets RunaDB SQL. Storage, transactions, WAL, and LSM correctness must not depend on an external database protocol.
...
- Independent RunaDB Client and RunaDB Server releases and boundaries are defined by ADR-0010.

## Migration and Validation

1. Publish the RunaDB protocol, RunaDB SQL, error model, and version-negotiation specifications with a minimal official CLI/driver.
...
Until steps 1–3 finish, the current implementation may still accept the PostgreSQL Frontend/Backend Protocol. That is implementation status, not an external RunaDB compatibility promise.
