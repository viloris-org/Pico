...
| DML | 单行 `INSERT`、`SELECT`、`UPDATE`、`DELETE` | 支持并验证 | `sql/exec.zig` |
| 查询 | `WHERE` 中 `=`、`AND`、`IS [NOT] NULL`，`LIMIT` / `OFFSET` | 支持并验证 | `sql/exec.zig` |
| 事务 | `BEGIN` / `COMMIT` / `ROLLBACK` | 明确拒绝 | `sql/exec.zig` |
| 索引 | `CREATE [UNIQUE] INDEX` / `DROP INDEX` | 明确拒绝 | `sql/exec.zig` |
| 约束 | 外键、表级唯一约束、复合主键、`CHECK` | 明确拒绝 | `sql/parse.zig` |
| DML | `RETURNING`、多行 `INSERT`、`ON CONFLICT` | 明确拒绝 | `sql/parse.zig` |
| 查询 | `ORDER BY`、`OR`、`NOT`、比较、`IN`、`LIKE`、聚合和分组 | 明确拒绝 | `sql/parse.zig` |
